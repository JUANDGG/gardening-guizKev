...
@Query("SELECT e.firstName, e.lastName1, e.lastName2, e.email FROM Employee e WHERE e.manager.employeeCode = 7")
...
@Query("SELECT e.firstName, e.lastName1, e.lastName2, e.position FROM Employee e WHERE e.position IS NOT NULL AND e.position <> 'Sales Representative'")
...
@Query("SELECT e, m.firstName AS managerName FROM Employee e LEFT JOIN e.client c LEFT JOIN e.manager m WHERE c.clientCode IS NULL")
...
@Query("SELECT e, o FROM Employee e JOIN e.office o LEFT JOIN e.client c WHERE c.clientCode IS NULL")
...
@Query("SELECT e FROM Employee e LEFT JOIN e.office o LEFT JOIN e.client c WHERE o.officeCode IS NULL AND c.clientCode IS NULL")
